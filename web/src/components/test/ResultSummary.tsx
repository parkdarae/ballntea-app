...
export const ResultSummary = ({
  ...
  return (
    <section className="retro-card flex flex-col gap-3 p-2.5 md:gap-8 md:p-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold tracking-tight text-ink md:text-5xl lg:text-6xl">
          Ball<span className="text-pop">&</span>Tea
        </h1>
        <p className="mt-0.5 text-xs uppercase tracking-[0.2em] text-lemon md:text-sm md:tracking-[0.6em]">
          TEST
        </p>
      </div>
      ...
      {mbtiDesc && (
        <section className="rounded-xl border border-pop/20 bg-gradient-to-br from-lemon/10 to-mint/10 p-2.5 md:rounded-3xl md:p-5">
          <div className="flex flex-wrap items-center gap-0.5">
            <span className="rounded-full bg-pop px-1.5 py-0.5 text-xs font-bold uppercase tracking-tight text-white md:px-3 md:py-1 md:text-sm">
              {result.mbtiType}
            </span>
            <h3 className="text-xs font-bold text-ink md:text-lg">{mbtiDesc.title}</h3>
          </div>
          <ul className="mt-1 space-y-0 text-xs leading-tight text-ink md:mt-3 md:space-y-1.5 md:text-base">
            {mbtiDesc.traits.map((trait, index) => (
              <li key={index} className="flex gap-0.5">
                <span className="mt-0.5 h-0.5 w-0.5 flex-shrink-0 rounded-full bg-pop" />
                <span>{trait}</span>
              </li>
            ))}
          </ul>
      ...
      <section className="space-y-1 md:space-y-3">
        <div>
          <h3 className="text-xs uppercase tracking-tight text-slate md:text-sm md:tracking-[0.3em]">
            🌈 에너지 방향성
          </h3>
          <p className="mt-0.5 text-xs leading-tight text-ink md:text-base">
      ...
      <section className="rounded-xl bg-ink px-2.5 py-1.5 text-white md:rounded-3xl md:px-5 md:py-4">
        <p className="text-xs uppercase tracking-tight text-lemon md:text-sm md:tracking-[0.3em]">
          Just for Today
        </p>
        <p className="mt-0.5 text-xs leading-tight md:text-base">
  ...
  <div className="rounded-xl border border-mint/30 bg-gradient-to-br from-mint/5 to-lemon/5 p-2.5 md:rounded-3xl md:p-4">
    <div className="flex items-center gap-0.5 md:gap-2">
      ...
      <div>
        ...
        <p className="text-xs uppercase tracking-tight text-slate md:tracking-[0.1em]">{subtitle}</p>
      </div>
    </div>
    <p className="mt-0.5 text-xs leading-tight text-ink">{description}</p>
    {traits && traits.length > 0 && (
      <ul className="mt-0.5 space-y-0 text-xs leading-tight text-ink">
        {traits.map((trait, index) => (
          <li key={index} className="flex gap-0.5">
            <span className="mt-0.5 h-0.5 w-0.5 flex-shrink-0 rounded-full bg-mint" />
            <span>{trait}</span>
          </li>
        ))}
      </ul>
...
const TeaCard = ({
  ...
  return (
    <article className="flex flex-col gap-0.5 rounded-xl border border-slate/15 bg-white/90 p-2.5 shadow-[0_12px_30px_rgba(27,43,82,0.08)] md:rounded-3xl md:p-4">
      <span className="w-fit rounded-full bg-lemon/40 px-1.5 py-0.5 text-xs font-semibold uppercase tracking-tight text-ink md:px-2.5 md:tracking-[0.15em]">
        {badge}
      </span>
      <div>
        <p className="text-xs font-semibold text-slate">{highlight}</p>
        <h3 className="text-xs font-bold text-ink md:text-lg">{tea.name}</h3>
        <p className="text-xs leading-tight text-slate">
          {tea.copy} · 카페인 {tea.caffeine ?? "정보 없음"}
        </p>
      </div>
      {tea.tasteNote && (
        <div className="flex flex-wrap gap-0.5 text-xs font-semibold text-slate">
